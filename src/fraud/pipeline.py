...
class Pipeline:
    ...
    async def _process_single(
        ...
        # Get the original image path for high-quality OCR
        original_image_path = file_path if file_path and Path(file_path).exists() else None

        # Get OCR data for font consistency
        ocr_data = None
        if original_image_path and invoice.document_type != DocumentType.TEXT_PDF:
            try:
                ocr_data = self._ocr_fallback.get_per_char_confidence_from_path(original_image_path)
                logger.info(f"OCR data retrieved for {invoice_id}: {len(ocr_data) if ocr_data else 0} words")
            except Exception as e:
                logger.error(f"Failed to get OCR data for {invoice_id}: {e}")

        # Export OCR data to JSON file and save image with bounding boxes
        if ocr_data and first_page_image:
            try:
                ocr_output_dir = self._config.font_consistency_dir
                ...
                ocr_output_path = ocr_output_dir / f"{invoice_id}_ocr.json"
                with open(ocr_output_path, "w", encoding="utf-8") as f:
                    json.dump(ocr_export, f, indent=2, ensure_ascii=False)
                logger.info(f"Exported OCR data to {ocr_output_path}")

                # Save image with bounding boxes (use original image for quality)
                self._save_ocr_bounding_boxes(original_image_path, ocr_data, invoice_id, ocr_output_dir)

            except Exception as e:
                logger.error(f"Failed to export OCR data: {e}")
        ...
        for idx, fields in enumerate(extracted_invoices):
            ...
            try:
                # Get first image path for high-quality export
                # Use original file_path if available, otherwise use saved path
                if file_path and Path(file_path).exists():
                    first_image_path = file_path
                else:
                    first_image_path = invoice.image_paths[0] if invoice.image_paths else None
    ...
    def _save_ocr_bounding_boxes(
        ...
        """Draw bounding boxes around OCR-detected words and save the image."""
        import numpy as np
        import cv2

        # Load original image from path for best quality
        if image_path and Path(image_path).exists():
            original_img = Image.open(image_path)
        else:
            logger.warning(f"Original image path not found: {image_path}")
            return
        ...
        for word_data in ocr_data:
            ...
            text = f"{conf}%"
            text_pos = (left, max(0, top - 5))
            cv2.putText(img_array, text, text_pos, cv2.FONT_HERSHEY_SIMPLEX,
                       0.5, (0, 255, 0), 1, cv2.LINE_AA)

        # Convert back to PIL and save
        img_bgr = cv2.cvtColor(img_array, cv2.COLOR_BGR2RGB)
        result_image = Image.fromarray(img_bgr)
        output_path = output_dir / f"{invoice_id}_ocr_boxes.png"
        result_image.save(output_path)
        logger.info(f"Saved OCR bounding boxes to {output_path}")
