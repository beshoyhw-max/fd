...
Return ONLY valid JSON."""
    ...
    def _parse_response_raw(self, response: str) -> List[Dict]:
        ...
        logger.debug(f"RAW response (first 200): {response[:200]}")

        # First, strip markdown fences that wrap JSON responses
        cleaned = response.strip()
        cleaned = re.sub(r"^```(?:json)?\s*\n?", "", cleaned)
        cleaned = re.sub(r"\n?```\s*$", "", cleaned)
        cleaned = cleaned.strip()

        # DEBUG: Log after fence removal
        logger.debug(f"After fence removal (first 200): {cleaned[:200]}")

        # Bulletproof JSON extractor: Finds everything from the first '{' to the last '}'
        # This handles any remaining wrapper text
        match = re.search(r"(\{.*\})", cleaned, re.DOTALL)

        if not match:
            logger.error(f"No JSON brackets found in LLM response: {cleaned[:500]}")
            raise ExtractionError("No JSON structure found in LLM response.")

        json_str = match.group(1)

        # DEBUG: Log the extracted JSON
        logger.debug(f"Extracted JSON (first 200): {json_str[:200]}")

        try:
            data = json.loads(json_str)
        except json.JSONDecodeError as e:
            logger.error(f"Cannot parse extracted JSON: {json_str[:500]}")
            raise ExtractionError(f"Invalid JSON from LLM: {e}")

        invoices_data = data.get("invoices")
        logger.debug(f"invoices_data found: {invoices_data}")

        if invoices_data is None:
            invoices_data = [data]

        if not invoices_data:
            logger.warning("No invoices found in parsed data")
            return []
    ...
    def _dict_to_fields(self, data: Dict) -> ExtractedFields:
        ...
        for item in data.get("line_items", []) or []:
            if isinstance(item, dict):
                # Forgiving key lookup to handle LLM schema hallucinations (e.g., using "total" instead of "line_total")
                line_total_val = item.get("line_total")
                if line_total_val is None:
                    line_total_val = item.get("total")
                if line_total_val is None:
                    line_total_val = item.get("amount")

                line_items.append(LineItem(
                    description=str(item.get("description", "")),
                    quantity=self._safe_float(item.get("quantity")),
                    unit_price=self._safe_float(item.get("unit_price")),
                    line_total=self._safe_float(line_total_val),
                ))
